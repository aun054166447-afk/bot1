...
def is_approved(chat_id):
    """Admin ဖြစ်စေ၊ approve ဖြစ်စေ True ပြန်မည်။"""
    return str(chat_id) == ADMIN_ID or approve.get(chat_id, False)
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, target=None, message=None, progress_msg=None, plan_filters=None):
    ...
    try:
        while True:
            ...
            # 🔥 Admin ဖြစ်ရင် key စစ်ဆေးမှုကို ကျော်သွားမည်
...
async def handle_key(message):
    ...
    # 🔥 Admin ဆိုရင် auto approve လုပ်ပြီး key မလိုပါ
    if str(chat_id) == ADMIN_ID:
        approve[chat_id] = True
        user_data.setdefault(chat_id, {})
        save_state()
        await bot.reply_to(message, "✅ Admin auto-approved. /setup ဖြင့် Session URL ထည့်ပါ။")
        return
...
async def handle_setup(message):
    ...
    if len(args) < 2:
        ...
        return
    url = args[1]
    # 🔥 Admin ဖြစ်စေ၊ approve ဖြစ်စေ ခွင့်ပြုမည်
