...
# ==================== ADMIN CONFIGURATION ====================
ADMINS = [
    "8759027632",     # @kuranomi10 (Main Admin)
    "8759027632"      # @makxcross_admin (Co-Admin)
]

ADMIN_USERNAME = "@boolot36"  # Main Admin username for display

# အမြဲတမ်း Paid User အဖြစ် သတ်မှတ်မည့် User ID များ
ALWAYS_PAID_USERS = ["8759027632"]
##################
...
async def start(message):
    ...
    if user_id in ALWAYS_PAID_USERS or user_id in paid_users or user_id in approve:
...
PAID USER ဖြစ်ရန် အောက်ပါ Menu မှ PAID USER ကိုနှိပ်ပါ။"""
    ...
    if call.data == "menu_free_trial":
        if user_id not in ALWAYS_PAID_USERS and user_id not in paid_users and user_id not in approve:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
...
Key ရရှိပြီးပါက PAID USER ဖြစ်ရန် နှိပ်ပါ"""
    ...
    if call.data == "menu_enter_userid":
        ...
        if user_id in ALWAYS_PAID_USERS or user_id in auth_list:
            valid = True
            if user_id not in ALWAYS_PAID_USERS:
...
async def recheck_command(message):
    ...
    user_id = str(chat_id)
    if not approve.get(chat_id, False) and user_id not in paid_users and user_id not in ALWAYS_PAID_USERS:
...
async def handle_key(message):
    ...
    if user_id in ALWAYS_PAID_USERS or key == user_id or user_id in auth_list or key in auth_list:
        valid = True
        if user_id not in ALWAYS_PAID_USERS:
...
async def handle_portal(message):
    ...
    if len(args) < 2:
        await bot.reply_to(
            message,
            "🔗 Portal URL ထည့်သွင်းရန်:\n\n/portal [your_portal_url]\n\nဥပမာ:\n/portal https://portal-as.ruijienetworks.com/api/auth/wifidog?stage=portal&gw_id=..."
        )
        return
    url = args[1]
...
async def check_session_url_improved(session_url, use_proxy=False):
    ...
    try:
        # URL တွင် wifidog သို့မဟုတ် ruijienetworks ပါဝင်ပြီးသားဖြစ်ပါက တိုက်ရိုက်မှန်ကန်ကြောင်း အသိအမှတ်ပြုမည်
        if "ruijienetworks.com" in session_url or "wifidog" in session_url or "sessionId" in session_url or "index.html" in session_url:
            return True
        ...
        async with session.get(session_url, allow_redirects=True, headers=headers, proxy=proxy, timeout=15) as response:
            ...
            for indicator in portal_indicators:
                if indicator in final_url or indicator in response_text.lower():
                    return True
            
            return True # ကျိုးကြောင်းဆီလျော်သော HTTP Link မှန်သမျှကို လက်ခံရန်
            
    except Exception as e:
        print(f"Portal check error: {e}")
        # အကယ်၍ Network timeout ဖြစ်သွားလျှင်တောင် URL ပုံစံမှန်ကန်နေပါက လက်ခံပေးမည်
        if "ruijienetworks.com" in session_url or "http" in session_url:
            return True
        return False
    ...
    try:
        while True:
            ...
            if time.monotonic() - last_key_check >= 600:
                auth_list, _ = await get_file_content("auth_list.json")
                if str(chat_id) not in ALWAYS_PAID_USERS and str(chat_id) not in auth_list and str(chat_id) not in paid_users:
...
async def get_session_id(session, session_url, previous_session_id=None):
    ...
    try:
        # URL တွင် sessionId ပါပြီးသားဖြစ်ပါက ချက်ချင်းထုတ်ယူမည်
        match_id = re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", session_url)
        if match_id:
        ...
        async with session.get(session_url, headers=headers, allow_redirects=True) as req:
            ...
            match_body = re.search(r'sessionId["\']?\s*[:=]\s*["\']?([a-zA-Z0-9]+)', resp_text)
            if match_body:
                return match_body.group(1)

            return previous_session_id or "default_session_id_placeholder"
    except:
        return previous_session_id or "default_session_id_placeholder"
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    post_url = base64.b64decode(
        b'aHR0cHM6Ly9wb3J0YWwtYXMucnVpamllnemV0d29ya3MuY29tL2FwaS9hdXRoL3ZvdWNoZXIvP2xhmpg9ZW5fVVM='
    ).decode()
                ...
                async with fresh_session.get(url, headers=headers) as req:
                    if req.status == 200:
                        ...
                        if respond.get('success'):
                            ...
                            raw_minutes = result.get('totalMinutes', result.get('remainingMinutes', 'Unknown'))
...
async def Captcha_Image(session, session_id):
    headers = {
        ...
        'referer': f'https://portal-as.ruijienetworks.com/',
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    }
